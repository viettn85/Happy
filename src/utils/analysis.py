...
from common import readFile, getConfigParser, readRawFile
import pandas as pd
import functools
    ...
    if not df.loc[str(date)].empty:
        ...
        if len(positions) > 0 and positions[0] < len(df) - 1:
            ...
            if "Bought" in df.iloc[position].Action and (stock not in portfolio.index):
                holidays = readRawFile(EXCEPTION_LOCATION + "holidays.csv")
                ic("{} {} ".format(str(date)[0:10],str(date)[0:10] in list(holidays.Date)))
                if str(date)[0:10] in list(holidays.Date):
                    df.Action.iloc[position] = "Relax"
                    df.Recommendation.iloc[position] = "Holiday! Not Buy Today"
                else:
                    price = getPrice(df.iloc[position], TRADE_STRATEGY)
...
def analyzeAll(date, files, portfolio, investingAmount):
    ...
    stocksToBuyMore = []
    for file in files:
        ic("Analyze {}".format(file[0:3]))
        logger.info("Analyze {}".format(file[0:3]))
